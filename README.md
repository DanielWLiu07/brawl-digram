...
`data/tile_sprites_official/` holds 356 named SVG tile sprites (walls, bushes, water, fences, spawns, springpads, etc) plus a preset mapping each `tiles.csv` tile char to its biome-variant assets. Sourced from [bloodwiing/BrawlMapGen](https://github.com/bloodwiing/BrawlMapGen) (GPLv3 code, Supercell fan-content art). The flat lookup `data/tile_sprite_index.json` is generated by `data/build_tile_sprite_index.py`. The prototype's map renderer reads this index and stamps the right SVG per tile char.

## License

All extracted assets and CSV data are Supercell IP. Used under the [Supercell Fan Content Policy](https://www.supercell.com/fan-content-policy). Non-commercial only. Tile SVGs originate from `bloodwiing/BrawlMapGen` (GPLv3); reused here for the same fan-content scope.
